...
We can access the local variable `width` from the lambda expression if it is **not** changed after initialization meaning it is `final` or _effectively final_. Member variables are passed by reference and do not need to be final or effectively final. We will talk more about _closures_ in the next module.
...
1. **Create a Class:** Create a class called `Person_Task_1` with fields `name` (String), `age` (int), and `city` (String).

2. **Create a List:** Create a list of `Person_Task_1` objects with various names, ages, and cities.

3. **Sort by Age:** Use lambda expressions and the `Comparator` interface to sort the list of `Person_Task_1` objects by age in ascending order.
